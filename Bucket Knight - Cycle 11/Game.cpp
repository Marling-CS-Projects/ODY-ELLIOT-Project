...
void Game::init(const char* title, int xpos, int ypos, int width, int height, bool fullscreen)
{
	...
	if (SDL_Init(SDL_INIT_EVERYTHING) == 0)
	{
		...
		if (rollSound == NULL || newLevel == NULL || menuClick == NULL || playerHit == NULL || enemyHit == NULL || shoot == NULL)
		{
		...
		for (auto cc : colliders)
		{
			...
			{	
				...
				if (p->hasComponent<ColliderComponent>()) {
					...
					if (cc->tag == "enemybullet" && Collision::AABB(p->getComponent<ColliderComponent>(), *cc))
					{
...
void Game::Attack()
{
	...
	{
		std::cout << "Attack on Cooldown" << std::endl;
	}
}
...
void Game::AttackGun()
{
	...
	bullet->CreateBullet(player->GetTransform().position.x + 12, player->GetTransform().position.y + 12, 12, 12, "Assets/Bullet.png", "playerbullet", dir, 3, 1, 400);
...
void Game::AttackMagic()
{
	...
	auto bullet = new Bullet();
	bullet->CreateBullet(player->GetTransform().position.x + 12, player->GetTransform().position.y + 12, 12, 12, "Assets/Magic.png", "playerbullet", dir, 3, 3, 500);
	bullet->entity->addGroup(groupBullets);
...
void Game::AttackMelee()
{
	...
	auto bullet = new Bullet();
	bullet->CreateBullet(player->GetTransform().position.x + 12 + (dir.x * 48), player->GetTransform().position.y + 12 + (dir.y * 48), 48, 48, "Assets/BONK.png", "playerbullet", dir, 0.2, 2, 3);
	bullet->entity->addGroup(groupBullets);
