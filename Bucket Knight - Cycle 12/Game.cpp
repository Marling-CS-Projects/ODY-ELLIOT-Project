...
Mix_Chunk* magic = nullptr;
Mix_Chunk* melee = nullptr;
Mix_Chunk* enemyHit = nullptr;
Mix_Chunk* playerHit = nullptr;
Mix_Chunk* menuClick = nullptr;
Mix_Chunk* newLevel = nullptr;
Mix_Chunk* rollSound = nullptr;
Mix_Chunk* emagic = nullptr;
Mix_Chunk* hair = nullptr;
...
auto canShoot = new Object();

int gunCooldown = 17;
int magicCooldown = 30;
int meleeCooldown = 13;

unsigned int frameToWaitFor;

Game* gameRef;

vector<Bullet*> btorender{};

int MouseX, MouseY = 0;
float w, h = 0;
float scaleX, scaleY = 1;

auto GameOver = new Trigger();
bool gameOver = false;
int framesforgameover = 200;
unsigned int frametowaitforgameover;

auto Pause = new Trigger();
bool paused = false;
bool canPause = true;
...
void Game::StartGame()
{
	...
	player = new Player();
	player->CreatePlayer(260, 320, 3);
	player->entity->addGroup(groupPlayers);

	bootIcon = new Object();
	bootIcon->CreateObject();
	bootIcon->entity->addComponent<TransformComponent>(720, 8, 36, 36, 2);
	bootIcon->entity->addComponent<SpriteComponent>("Assets/DashIcon.png");
	bootIcon->entity->addComponent<RollIcon>();
	bootIcon->entity->addGroup(groupForeground);
	...
	auto wall1 = new Wall(204, 120, 432, 24, "Assets/WallTexture.png", "wally");
	wall1->entity->addGroup(groupForeground);

	auto wall2 = new Wall(204, 528, 432, 24, "Assets/WallTexture.png", "wally");
	wall2->entity->addGroup(groupForeground);

	auto wall3 = new Wall(612, 120, 24, 432, "Assets/WallTexture.png", "wallx");
	wall3->entity->addGroup(groupForeground);

	auto wall4 = new Wall(204, 120, 24, 432, "Assets/WallTexture.png", "wallx");
	wall4->entity->addGroup(groupForeground);

	auto hundreds = new Object();
	hundreds->CreateObject();
	hundreds->entity->addComponent<TransformComponent>(345, 12, 24, 24, 2);
	hundreds->entity->addComponent<SpriteComponent>("Assets/0.png");
	hundreds->entity->addComponent<ScoreComponent>("hundreds");
	hundreds->entity->addGroup(groupScore);

	auto tens = new Object();
	tens->CreateObject();
	tens->entity->addComponent<TransformComponent>(395,12,24,24,2);
	...
	ones->CreateObject();
	ones->entity->addComponent<TransformComponent>(445, 12, 24, 24, 2);
	...
	canShoot = new Object();
	canShoot->CreateObject();
	canShoot->entity->addComponent<TransformComponent>(728, 520, 14, 14, 4);
	canShoot->entity->addComponent<SpriteComponent>("Assets/MagicOFF.png");
	canShoot->entity->addComponent<CanShootIcon>(weapon);
	canShoot->entity->addGroup(groupForeground);

	dGen = new Dungeon(256);
	dGen->GenerateLayout();
	dGen->DrawCurrentRoom();

	std::srand(time(NULL));
	int randWeapon = std::rand() % 3;

	if (randWeapon <= 0)
	{
		gameRef->SwitchWeapon("Gun");
	}
	else if (randWeapon == 1)
	{
		gameRef->SwitchWeapon("Magic");
	}
	else
	{
		gameRef->SwitchWeapon("Melee");
	}
}
...
void Game::init(const char* title, int xpos, int ypos, int width, int height, bool fullscreen)
{
	...
	if (SDL_Init(SDL_INIT_EVERYTHING) == 0)
	{
		...
		Mix_VolumeMusic(2.8f);

		shoot = Mix_LoadWAV("Assets/shoot.wav");
		shoot->volume = 3;

		magic = Mix_LoadWAV("Assets/magic.wav");
		magic->volume = 3;

		melee = Mix_LoadWAV("Assets/melee.wav");
		melee->volume = 3;
		...
		emagic = Mix_LoadWAV("Assets/emagic.wav");
		emagic->volume = 3;

		hair = Mix_LoadWAV("Assets/hair.wav");
		hair->volume = 3;
		...
		rollSound = Mix_LoadWAV("Assets/rollSound.wav");
		rollSound->volume = 4;

		gameover = Mix_LoadWAV("Assets/GameOver.wav");
		gameover->volume = 4;

		pause = Mix_LoadWAV("Assets/pause.wav");
		pause->volume = 4;

		if (rollSound == NULL || newLevel == NULL || menuClick == NULL || playerHit == NULL || enemyHit == NULL || shoot == NULL || gameover == NULL || pause == NULL)
		{
		...
		GameOver = new Trigger(311, 240, 43, 28, 4, "Assets/GameOver.png", "gameOver");
		Pause = new Trigger(292, 240, 62, 14, 4, "Assets/Paused.png", "gameOver");
...
void Game::handleEvents()
{
	...
	if (Game::event.type == SDL_MOUSEBUTTONDOWN)
	{
		...
		if (SDL_BUTTON_RIGHT == Game::event.button.button && gameStarted)
		{
			for (auto& p : players)
			{
				p->getComponent<PlayerComponent>().roll(rollSound);
			}
		}
	}
	...
	if (Game::event.type == SDL_KEYDOWN)
	{
		...
		case SDLK_p:
			if (gameStarted && !paused && canPause)
			{
				paused = true;
				canPause = false;
			...
			else if (gameStarted && paused && canPause)
			{
				paused = false;
...
void Game::update()
{
	...
	for (auto b : btorender)
	{
		b->entity->addGroup(groupBullets);
	}
	btorender.clear();
	...
	if (enemies.size() <= 0 && gameStarted)
	{
		...
		btorender.clear();
		enemies.clear();
		player->entity->getComponent<TransformComponent>().previousPos = Vector2D(260, 320);
		player->entity->getComponent<TransformComponent>().position = Vector2D(260, 320);
		dGen->DrawCurrentRoom();
	}
	
	if (gameStarted && !paused)
	{
		Pause->entity->delGroup(groupMenu);
		bootIcon->entity->getComponent<RollIcon>().on = player->entity->getComponent<PlayerComponent>().canRoll();
		...
		for (auto cc : colliders)
		{
			...
			{	
				...
				if (p->hasComponent<ColliderComponent>()) {
					if (cc->tag == "wallx" && Collision::AABB(p->getComponent<ColliderComponent>(), *cc))
					{
						p->getComponent<TransformComponent>().position.x = p->getComponent<TransformComponent>().previousPos.x;
					}
					if (cc->tag == "wally" && Collision::AABB(p->getComponent<ColliderComponent>(), *cc))
					{
						p->getComponent<TransformComponent>().position.y = p->getComponent<TransformComponent>().previousPos.y;
					}

					if (cc->tag == "enemybullet" && Collision::AABB(p->getComponent<ColliderComponent>(), *cc) && !p->getComponent<PlayerComponent>().rolling)
					{
						p->getComponent<PlayerComponent>().hit();
			...
			for (auto& e : enemies)
			{
				if (e->hasComponent<ColliderComponent>())
				{
					if (cc->tag == "playerbullet" && Collision::AABB(e->getComponent<ColliderComponent>(), *cc))
					{
						...
						{
							for (int i = 0; i < cc->entity->getComponent<ProjectileComponent>().GetDamage(); i++)
							{
								if (e->isActive())
								{
									if (e->hasComponent<MeleeEnemy>())
									{
										e->getComponent<MeleeEnemy>().hit();
									}
									else if (e->hasComponent<MagicEnemy>())
									{
										e->getComponent<MagicEnemy>().hit();
									}
									else if (e->hasComponent<GunEnemy>())
									{
										e->getComponent<GunEnemy>().hit();
									}
					...
					if (cc->tag == "player" && Collision::AABB(e->getComponent<ColliderComponent>(), *cc) && e->hasComponent<MeleeEnemy>() && !cc->entity->getComponent<PlayerComponent>().rolling)
					{
						...
						cc->entity->getComponent<PlayerComponent>().hit();
			...
			for (auto& b : bullets)
			{
				if (Collision::AABB(b->getComponent<ColliderComponent>(), *cc) && (cc->tag == "wally" || cc->tag == "wallx"))
				{
	...
	{
		Pause->entity->addGroup(groupMenu);
	}
	...
	if (gameOver && frametowaitforgameover <= gameRef->GetFrames())
	{
		gameOver = false;
		Game::EndGame();
	}
	...
	{
		GameOver->entity->addGroup(groupMenu);
	...
	if (frameToWaitFor <= gameRef->GetFrames() && gameStarted)
	{
		if (canShoot->entity->hasComponent<CanShootIcon>())
		{
			canShoot->entity->getComponent<CanShootIcon>().on = true;
		}
	}
	...
	else if (player->entity->hasComponent<InputController>())
	{
		player->entity->getComponent<InputController>().update();
	}
}
...
void Level::SpawnEnemies()
{
	...
	for (int i = 0; i < this->numberOfEnemies; i++)
	{
		...
		if (k <= 0)
		{ 
			auto mE = new EMagic(x, y, 24, 24, "Assets/Wizard.png", "enemy", &p);
			mE->entity->addGroup(groupEnemies);
		}
		else if (k == 1)
		{
			auto mE = new EGun(x, y, 24, 24, "Assets/Fluffball.png", "enemy", &p);
			mE->entity->addGroup(groupEnemies);
		}
		else
		{	
...
void Game::CreateEnemyBullet(int x, int y, int width, int height, const char* texture, std::string tag, Vector2D dir, float speed, float damage, float distance, const char* enemyType)
{
	auto bullet = new Bullet(x, y, width, height, texture, tag, dir, speed, damage, distance);

	btorender.push_back(bullet);

	if (enemyType == "Magic")
	{
		Mix_PlayChannel(-1, emagic, 0);
	}
	else
	{
		Mix_PlayChannel(-1, hair, 0);
	}
...
void PlayerComponent::StopGame()
{
	...
	this->entity->destroy();
	frametowaitforgameover = gameRef->GetFrames() + framesforgameover;
	gameOver = true;
...
void Game::EndGame()
{
	for (auto m : menu)
	{
		if (m->getComponent<ColliderComponent>().tag != "gameOver")
		{
			m->destroy();
		}
	}
...
void Game::Attack()
{
	if (canShoot->entity->getComponent<CanShootIcon>().on && !paused)
	{
		if (weapon == "Gun")
		{
			AttackGun();
...
void Game::AttackGun()
{
	Vector2D dir{ (float)MouseX - player->GetTransform().position.x - 8, (float)MouseY - player->GetTransform().position.y - 20 };
	dir = dir.normalize(dir);
	auto bullet = new Bullet();
	bullet->CreateBullet(player->GetTransform().position.x + 8, player->GetTransform().position.y + 20, 12, 12, "Assets/Bullet.png", "playerbullet", dir, 4.5f, 1, 400);
...
void Game::AttackMagic()
{
	...
	auto bullet = new Bullet();
	bullet->CreateBullet(player->GetTransform().position.x + 8, player->GetTransform().position.y + 20, 12, 12, "Assets/Magic.png", "playerbullet", dir, 4, 2, 500);
	bullet->entity->addGroup(groupBullets);
...
{
	Vector2D dir{ (float)MouseX - player->GetTransform().position.x + 4, (float)MouseY - player->GetTransform().position.y + 10 };
	dir = dir.normalize(dir);

	auto bullet = new Bullet();
	bullet->CreateBullet(player->GetTransform().position.x - 4 + (dir.x * 48), player->GetTransform().position.y - 10 + (dir.y * 48), 48, 48, "Assets/BONK.png", "playerbullet", dir, 0.2, 3, 3);
	bullet->entity->addGroup(groupBullets);

	Mix_PlayChannel(-1, melee, 0);
}

void Game::SwitchWeapon(const char* s)
{
	if (canShoot->entity->hasComponent<SpriteComponent>())
	{
		if (s != "")
		{
			canShoot->entity->getComponent<SpriteComponent>().setTex(s);
			weapon = s;
			canShoot->entity->addComponent<CanShootIcon>(weapon);
		}
	}
